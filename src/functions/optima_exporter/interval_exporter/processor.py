...
def process_site(
    ...
    """
    Process a single site: download the interval ZIP, extract CSV, and upload it to S3.

    Sentinel "No data is available" CSVs are still uploaded for audit retention; the
    result["empty_data"] flag is set so callers can count them separately.

    Returns:
        Dict with at minimum: nmi, site_id, success, error.
        On success also: filename, s3_key, empty_data (bool).
    """
    result: dict[str, Any] = {
        "nmi": nmi,
        "site_id": site_id_str,
    ...
    try:
        ...
        logger.error(
            "Interval ZIP extraction failed",
            exc_info=True,
    ...
    result["empty_data"] = b"No data is available" in csv_content
    return result
