...
class NEMFile:
    ...
    def get_data_frame(self, split_days: bool = False, set_interval: int = 0) -> pd.DataFrame | None:
...
def parse_300_row(row: list, interval: int, uom: str, meter_serial_number: str) -> IntervalRecord:
    ...
    if len(row) < num_intervals + num_required_non_reading_fields:
        ...
        raise ValueError(f"Unexpected number of values in 300 row: {num_rows} readings for {interval}min intervals")
